...
## Google Play Data Safety — entered selections; save failed

Root filled all ten types below and verified the preview. Save and Save as draft / Save Draft retries returned unexpected error **`5C897C1E`**; a fresh dashboard returned **403 Forbidden**. The original filled form remains open in browser tab 15, with the failed dashboard in tab 16. These are entered answers, **not verified saved declarations**. Resume from that form after provider access recovers and confirm persistence before recording completion.
...
The authorized map disclosure is live at the canonical policy URL in dedicated mobile deploy `6a9c9097c7e4b26b67fadcd5`, verified at 21:59 UTC on 5 September 2026 (6 September EAT). HTTP 200 policy bytes match source SHA-256 `e6c4606a95cfc0083111bf13fad61aac82769fcbcf9cdacca5a4447c108cdb18`; SHA-1 `bea6907ae99111038b5d5db391ed431930c6dc48`. It explicitly identifies OSMF as an independent controller and explains retained country-level IP inference and operational, security and usage-analysis purposes. No precise retention period or GPS collection is claimed.

Live WeatherKit returned 200 with observation `2026-09-05T21:59:16Z`; marine returned 200/unavailable; report GET returned 405 without email. Three function ZIPs and backend source hashes were preserved. All seven required production/functions settings were checked before and after deployment, with no environment changes. Original website deploy `6a8dbf35cb01b33e235abc4b` and local linked-site state are unchanged. This policy publication did not change app source behavior, store settings or send an external message.
