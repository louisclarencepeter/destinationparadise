...
> **Source-control reconciliation — 8 September.** All ten mobile commits were transferred, in order, onto remote `development` in an isolated worktree. This handoff, `design/mobile-app/`, and the preserved release evidence are now included in that history. The nine older local website commits and unfinished website edits were preserved, then reviewed in the subsequent [website reconciliation](../WEBSITE_HANDOFF.md). Useful website changes are now integrated with current development; the original history and working state remain recoverable in the documented bundle and stash. Start further shared work from current `development`, use temporary branches only as needed, merge back into `development`, and delete completed temporary branches. Do not open a `development` → `main` PR without an explicit user request. See [RECONCILIATION.md](release/RECONCILIATION.md) for the commit mapping and validation scope.
